...
constexpr color32 powerToColor(const int power, bool white_buck = false)
{
...
struct RayCaster
{
	...
	const color32 cbord = color32{ 49, 160, 255, 255 };
	...
	void throwRay(Ray& ray, World& world)
	{
		...
		while (power > 0)
		{
			...
			if (*colizion.raw_data((int)pos.x, (int)pos.y) == cbord)
				break;
			...
			if (clHor || clVer)
			{
				...
				ref_ray.dir =
					math::move_on_angle({ 0,0 }, math::RandomStdReal(-9., 9.) + math::anlage_2d({ 0,0 }, ref_ray.dir), 0.99999f);
			...
			if (world.walls.raw_data((int)pos.x, (int)pos.y)->r > 90)
			{
				...
				int dist = (int)math::distance_2d(world.wifi_pos, pos);
...
public:
	void loadPlan(const std::string& path)
	{
		...
		walls = png_read_image_file(path);
...
void imview::wifiMap(view_context*)
{
	const auto canvasPosGlob  = ImGui::GetCursorPos() + ImGui::GetWindowPos();
	const auto canvasAvail    = ImGui::GetContentRegionAvail();
	      auto canvasDrawlist = ImGui::GetWindowDrawList();

	const FLOAT2 globalMousePos =
	{
    ...
    const auto color_yellow = IM_COL32(255, 255,   0,  90);
	...
	static       bitmap32 color_buffer(size2.x, size2.y, color32{ 255, 255, 0, 64 });
	...
	ImGui::BeginGroup();
	{
		ImGui::PushItemWidth(200);
		{
			...
			if (ImGui::Button("ast/wifi/A+", { 90, 0 })) { simulator.loadPlan(ASSET_DIR + "wifi/walls.png"); }
			if (ImGui::Button("ast/wifi/test", { 90, 0 })) { simulator.loadPlan(ASSET_DIR + "wifi/A.png"); }
			if (ImGui::Button("ast/wifi/null", { 90, 0 })) { simulator.loadPlan(ASSET_DIR + "wifi/B.png"); }
			...
			if (ImGui::Button("Start", { 90, 0 }))
			{
				for (;;)
				{
					...
					{
						log::warn << IN_UTF8("Don`t forget to set load plan");
						break;
					}

					if (math::equal(simulator.wifi_spot.Pos, FLOAT2{ 0, 0 }))
					{
						log::warn << IN_UTF8("Don`t forget to set wifi position");
						break;
					}
	...
	if ("wifi position")
	{
		...
		if (!math::equal(simulator.wifi_spot.Pos, { 0,0 }))
		{
			ImGui::GetForegroundDrawList()->AddRectFilled(
				canvasPosGlob + simulator.wifi_spot.Pos - FLOAT2{ 8,8 },
				canvasPosGlob + simulator.wifi_spot.Pos + FLOAT2{ 8,8 },
